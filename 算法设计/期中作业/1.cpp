...
Block Game::once_place() {
    ...
    int max_layer = 0;
    std::array<int, ColumnBound> col_heights{}; // 存储每一列的高度
    ...
    for (int x = 0; x < ColumnBound; ++x) {
        for (int y = LineBound; y >= 1; --y) {
            if (map[y][x]) {
                ...
                current_agg_height += col_heights[x];
    ...
    for (int rotation = 0; rotation < 4; ++rotation) {
        ...
        for (int dx = min_x; dx <= max_x; ++dx) {
            ...
            int holes = 0;
            int bumpiness = 0;
            std::array<int, ColumnBound> col_heights_after{};
            bool roof[ColumnBound] = {};  // 是否有顶部已填充
            ...
            for (int x = 0; x < ColumnBound; ++x) {
                ...
                for (int y = LineBound; y >= 1; --y) {
                    if (preview[y][x]) {
                        col_heights_after[x] = LineBound - y + 1;
                        actual_land_height = std::min(actual_land_height, LineBound - y);
                        break;
                    }
                }
                new_agg_height += col_heights_after[x];
            }
