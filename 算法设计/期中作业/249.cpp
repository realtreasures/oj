...
public:
    ...
    static int calculateBumpiness(const BackMap& map) {
     ...
     // 新增方法：计算最大列高
     static int calculateMaxHeight(const BackMap& map) {
        auto heights = getColumnHeights(map);
        return *std::max_element(heights.begin(), heights.end());
    }

    // 新增方法：计算井深
    static int calculateWellDepth(const BackMap& map) {
        std::array<int, ColumnBound> heights = getColumnHeights(map);
        int wellDepth = 0;
        for (int col = 1; col < ColumnBound - 1; ++col) {
            int minNeighbor = std::min(heights[col-1], heights[col+1]);
            if (heights[col] < minNeighbor) {
                wellDepth += (minNeighbor - heights[col]);
            }
        }
        return wellDepth;
    }
...
std::vector<int> getValidRotations(BlockType type) {
    switch(type) {
        case BlockType::One:     return {0};
        case BlockType::Two:     return {0, 1};
        case BlockType::Corner:  return {0, 1, 2, 3};
        case BlockType::Square:  return {0};
        case BlockType::LeftL:   return {0, 1, 2, 3};
        case BlockType::RightL:  return {0, 1, 2, 3};
        case BlockType::Column:  return {0, 1};
        case BlockType::Cross:  return {0};
        default:                return {0};
    }
}
constexpr int WELL_DEPTH_WEIGHT = -4;
...
Block Game::once_place() {
    ...
    // 动态权重调整
    int currentMaxHeight = MapAnalyzer::calculateMaxHeight(map);
    int dynamicHeightWeight = currentMaxHeight > 15 ? -10 : -7;

    // 遍历所有可能的旋转
    for (int r : getValidRotations(original.type())) {
        Block rotated = original.remake_with(r);
        
        // 计算有效移动范围
        int left = rotated.lbound();
        int right = rotated.rbound();
        int max_dx_left = left;
        int max_dx_right = ColumnBound - right;
        
        for (int dx = -max_dx_left; dx <= max_dx_right; ++dx) {
            Block temp = rotated;
            if (!temp.move(dx)) continue;

            int removed_lines;
            BackMap preview = map.preview_rc(temp, removed_lines);
            ...
            int wellDepth = MapAnalyzer::calculateWellDepth(preview);

            // 动态权重计算
            int score = 
                dynamicHeightWeight * aggregateHeight +
                LINES_WEIGHT * removed_lines +
                HOLES_WEIGHT * holes +
                BUMPINESS_WEIGHT * bumpiness +
                WELL_DEPTH_WEIGHT * wellDepth;
