...
Block Game::once_place() {
    ...
    Block best_block = original;
    constexpr int DEATH_LAYER = 18;

    // 评估函数权重
    constexpr int LANDING_HEIGHT_WEIGHT = -45;
    constexpr int ERODED_WEIGHT = 34;
    constexpr int ROW_TRANS_WEIGHT = -32;
    constexpr int COL_TRANS_WEIGHT = -98;
    constexpr int BURIED_HOLES_WEIGHT = -79;
    constexpr int WELLS_WEIGHT = -34;

    // 生成所有可能状态
    std::vector<Block> candidates;
    for (int r = 0; r < BlockRotationCount; ++r) {
        Block rotated = original.remake_with(r);
        
        // 获取水平移动范围
        int left = rotated.lbound();
        int right = rotated.rbound();
        for (int dx = -left; dx < (ColumnBound - right); ++dx) {
            Block temp = rotated;
            if (temp.move(dx)) {
    ...
    for (Block& candidate : candidates) {
        // 生成预览地图
        BackMap preview = map.preview(candidate);
        if (preview.max_layer() > DEATH_LAYER) continue;

        // 计算各项指标
        int landing_height = LineBound - candidate.get_x();
        int eroded_lines = 0;
        int row_trans = 0;
        int col_trans = 0;
        int buried_holes = 0;
        ...
        for (int x = 0; x < ColumnBound; ++x) {
            int left = (x > 0) ? x-1 : -1;
            int right = (x < ColumnBound-1) ? x+1 : -1;
            
            for (int y = 1; y < LineBound; ++y) {
                if (!preview[y][x] && 
                    (left == -1 || preview[y][left]) && 
                    (right == -1 || preview[y][right])) {
                    ++wells;
                }
            }
        }
        ...
        // 计算综合得分
        int score = LANDING_HEIGHT_WEIGHT * landing_height +
                   ERODED_WEIGHT * eroded_lines +
                   ROW_TRANS_WEIGHT * row_trans +
                   COL_TRANS_WEIGHT * col_trans +
                   BURIED_HOLES_WEIGHT * buried_holes +
                   WELLS_WEIGHT * wells;

        // 选择最优解
        if (score > max_score || 
           (score == max_score && candidate.get_x() < best_block.get_x())) {
            max_score = score;
