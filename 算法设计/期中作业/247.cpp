...
// 在文件头部的包含部分添加 <climits>
#include <climits>  // 新增此行解决 INT_MIN/MAX 问题
...
#include <cmath>
#include <vector>
#include <climits>  // 确保这里包含（如果已在上方添加可省略）
// 添加在全局作用域的常量定义
constexpr float LINES_WEIGHT = 1.0f;
constexpr float HOLES_WEIGHT = -0.5f;
constexpr float BUMPINESS_WEIGHT = -0.2f;
...
public:
    static int calculateAggregateHeight(const BackMap& map) {
        ...
        for (int col = 0; col < ColumnBound; ++col) {
            for (int row = map.max_layer(); row >= 1; --row) {
                if (map[row][col]) {
                    aggregate += row;
                    break;
                }
    ...
    static int calculateBumpiness(const BackMap& map) {
        ...
        for (int col = 0; col < ColumnBound; ++col) {
            ...
            if (col > 0) bump += std::abs(height - prev);
            prev = height;
        }
        return bump;
    }
...
Block Game::once_place() {
    ...
    constexpr int DEATH_LAYER = 18;
    constexpr int WARNING_LAYER = 15;

    const int current_max_layer = map.max_layer();
    const float height_weight = current_max_layer > WARNING_LAYER ? -10.0f : -5.0f;
    constexpr float layer_weight = -20.0f;

    const std::array<int, 4> base_rotations = {0,1,2,3};
    std::vector<int> rotations;
    switch(original.type()) {
        case BlockType::One:   rotations = {0}; break;
        case BlockType::Two:   rotations = {0,1}; break;
        case BlockType::Column:rotations = {0,1}; break;
        default:               rotations.assign(base_rotations.begin(), base_rotations.end());
    }

    int best_preview_max_layer = INT_MAX;
    int best_center_dist = INT_MAX;

    for (int r : rotations) {
        Block rotated = original.remake_with(r);
        const int width = rotated.rbound() - rotated.lbound() + 1;
        const int max_shift = std::min(10 - width, 10);
        for (int dx = -max_shift; dx <= max_shift; ++dx) {
            Block temp = rotated;
            if (!temp.move(dx)) continue;

            int removed = 0;
            BackMap preview = map.preview_rc(temp, removed);
            const int result_max_layer = preview.max_layer();
            if (result_max_layer > DEATH_LAYER) continue;

            const int layer_penalty = std::pow(result_max_layer - current_max_layer, 2);
            const int height_penalty = MapAnalyzer::calculateAggregateHeight(preview);
            const int bumpiness = MapAnalyzer::calculateBumpiness(preview);

            float score = 
                height_weight * height_penalty +
                layer_weight * layer_penalty +
                LINES_WEIGHT * removed * 10.0f +
                HOLES_WEIGHT * MapAnalyzer::calculateHoles(preview) +
                BUMPINESS_WEIGHT * bumpiness;

            if (result_max_layer < current_max_layer) score += 50.0f;

            const int current_center_dist = std::abs(temp.get_x() - (10 - width)/2);
            if (score > maxScore || 
               (score == maxScore && (
                   (result_max_layer < best_preview_max_layer) ||
                   (result_max_layer == best_preview_max_layer && 
                    current_center_dist < best_center_dist)
               )) 
            ) {
                maxScore = score;
                bestBlock = temp;
                best_preview_max_layer = result_max_layer;
    ...
    if (maxScore == INT_MIN) {
        for (int r : rotations) {
            Block temp = original.remake_with(r);
            while (temp.move(-1)) {
                BackMap preview = map.preview(temp);
                if (preview.max_layer() <= DEATH_LAYER) return temp;
            }
        }
        return original;
    }
