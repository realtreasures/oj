...
/* 你可以更改这行以下的代码们 */
#include <vector> // 确保在函数前包含头文件

Block Game::once_place() {    
    Block original_block = blocks[0];
    int max_removed = -1;
    Block best_block = original_block;
    const int DEATH_LAYER = 18;

    // 生成可能的旋转配置（已修复vector使用）
    std::vector<int> rotations;
    switch (original_block.type()) {
        case BlockType::One: rotations = {0}; break;
        case BlockType::Two: rotations = {0, 1}; break;
        case BlockType::Corner: rotations = {0, 1, 2, 3}; break;
        case BlockType::Square: rotations = {0}; break;
        case BlockType::LeftL: rotations = {0, 1, 2, 3}; break;
        case BlockType::RightL: rotations = {0, 1, 2, 3}; break;
        case BlockType::Column: rotations = {0, 1}; break;
        case BlockType::Cross: rotations = {0}; break;
        default: rotations = {0}; break;
    }

    // 第一阶段：寻找最佳消除方案
    for (int r : rotations) {
        Block rotated_block = original_block.remake_with(r);
        for (int dx = -10; dx <= 10; ++dx) {
            Block temp = rotated_block;
            if (temp.move(dx)) {
                int removed = 0;
    ...
    // 第二阶段：无法消除时的安全放置（已移除Candidate结构体）
    if (max_removed <= 0) {
        int min_layer = 999;
        Block safest_block = original_block;
        ...
        for (int r : rotations) {
            ...
            for (int dx = -10; dx <= 10; ++dx) {
                ...
                if (temp.move(dx)) {
                    ...
                    int current_layer = preview.max_layer();
                    
                    if (current_layer < min_layer) {
                        min_layer = current_layer;
                        safest_block = temp;
                    } else if (current_layer == min_layer) {
                        // 选择更居中的位置
                        int current_width = safest_block.rbound() - safest_block.lbound();
                        int ideal_pos = (10 - current_width) / 2;
                        
                        if (abs(temp.get_x() - ideal_pos) < 
                           abs(safest_block.get_x() - ideal_pos)) {
                            safest_block = temp;
                        }
                    }
                }
